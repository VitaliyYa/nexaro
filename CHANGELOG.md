...
- **Frontend SPA & PWA (Vue 3, Vite, Tailwind CSS, TypeScript):**
  ...
  - Карточки устройств для смарт-замков TTLock (с отображением статусов Locked/Unlocked и управлением PIN-кодами), реле освещения (Включено/Выключено), кранов перекрытия воды (Открыт/Перекрыт) с защитой от протечек, кондиционеров.
  - Раздел "Журнал событий и аудит": постраничная загрузка append-only телеметрии из `device_logs` через REST API.
